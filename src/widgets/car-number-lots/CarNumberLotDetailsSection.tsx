...
export default function CarNumberLotDetailsSection() {
  const { id } = useParams<{ id: string }>()
  const [item, setItem] = useState<CarNumberLot | null>(null)
  ...
  useEffect(() => {
    ...
    carNumberLotsApi
      ...
      .catch((err) => {
        if (!mounted) return
        const message = err?.response?.data?.message || err?.message || "Не удалось загрузить объявление"
        setError(message)
      })
  ...
  const publishedDate = publishedAt ? new Date(publishedAt).toLocaleDateString("ru-RU") : ""
  ...
  const phone = item.phone || item.sellerInfo?.phoneNumber || "—"
