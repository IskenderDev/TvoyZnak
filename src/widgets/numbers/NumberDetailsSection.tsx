...
export default function NumberDetailsSection() {
  ...
  return (
    <>
      ...
      <section className="bg-[#0b0b0c] py-6 desktop:py-8 desktop:py-10 text-white">
        <div className="mx-auto w-full max-w-[1220px] px-4 desktop:px-6 desktop:px-8">
          <div className="flex items-center gap-3 desktop:gap-4">
            <Link to={paths.home}>
              <button
                ...
                onClick={() => navigate(-1)}
                aria-label="Назад"
                className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#0177FF] shadow-[0_10px_30px_rgba(1,119,255,0.6)] transition-colors hover:bg-[#0C8BFF] desktop:h-11 desktop:w-11"
              >
                <LuArrowLeft className="h-5 w-5 desktop:h-6 desktop:w-6 text-white" />
              </button>
            </Link>

            <h1 className="flex flex-wrap items-baseline gap-x-1 gap-y-1 text-[22px] font-bold leading-[1.2] tracking-[0.01em] desktop:text-[26px] desktop:text-[30px] desktop:text-[36px]">
          ...
          <div className="mt-6 desktop:mt-8 rounded-[32px] border border-[#151515] bg-[#0b0b0c] px-3 py-5  desktop:px-6 desktop:py-6 desktop:px-10 desktop:py-8">
            <div className="flex justify-center px-1 desktop:px-4 desktop:px-8">
              <PlateStaticLg
                data={{
            ...
            <div className="mt-6 desktop:mt-8 grid gap-5 desktop:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] desktop:gap-6 desktop:gap-8">
              <div className="rounded-[24px] border border-[#252525] bg-[#0b0b0c] px-4 py-4  desktop:px-6 desktop:py-5 desktop:px-7 desktop:py-6">
                <dl className="divide-y divide-[#262626]">
                  {detailsRows.map((row) => (
                    <div
                      key={row.label}
                      className="flex items-center justify-between py-3 desktop:py-3.5"
                    >
                      <dt className="pr-4 text-[14px] font-semibold leading-[18px] text-white desktop:text-[15px]">
                        {row.label}
                      </dt>
                      <dd className="text-right text-[16px] leading-[20px] text-[#F5F5F5] desktop:text-[18px] desktop:leading-[22px] font-normal">
                ...
                <div className="mt-4 border-t border-[#262626] pt-4 desktop:mt-5 desktop:pt-5 flex flex-wrap items-center justify-between gap-x-4 gap-y-3">
                  <div className="text-[22px] font-medium leading-none desktop:text-[26px] desktop:text-[30px]">
                    {price}
                  </div>

                  <button
                    onClick={handleBuyClick}
                    className="inline-flex min-h-[44px] min-w-[150px] items-center justify-center rounded-full bg-[#0177FF] px-6 py-2.5 text-[15px] font-medium leading-[18px] tracking-[0.01em] text-white shadow-[0_10px_30px_rgba(1,119,255,0.6)] transition-colors hover:bg-[#0C8BFF] desktop:min-h-[46px] desktop:px-7 desktop:text-[16px] desktop:min-h-[48px] desktop:px-8 desktop:text-[18px] desktop:leading-[22px]"
                  >
                    Купить
                  </button>
                </div>
              ...
              <div className="rounded-[24px] border border-[#252525] bg-[#0b0b0c] px-4 py-4 text-[15px] leading-[1.5] text-[#E5E5E5] desktop:px-6 desktop:py-5 desktop:px-7 desktop:py-6 desktop:text-[16px] desktop:text-[18px] desktop:leading-[1.55]">
              
                <h2 className="sr-only">Важная информация</h2>
                <ul className="list-disc space-y-3 desktop:space-y-3.5 pl-5 marker:text-[#0177FF]">
                  <li>
